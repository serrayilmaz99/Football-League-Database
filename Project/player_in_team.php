...
<body>
  ...
  $sql_statement = "INSERT INTO `player_plays_in_team` (playerID, teamID, contractStartDate, contractEndDate,fee  )
              VALUES ($playerID, $teamID ,'$contractStartDate', '$contractEndDate' , $fee)";
  ...
  // database insert SQL code
  $sql_statement = "DELETE FROM `player_plays_in_team`
  ...
  echo "Error: " . $sql_statement . "<br>" . $db->error;
    ...
    if(! (  (isset($_POST['playerID'])) && (strlen($_POST['playerID']) != 0) ) || (! (  (isset($_POST['teamID'])) && (strlen($_POST['teamID']) != 0)) )  ){
      ?>
      <body style="background-color:#f2f4f7;">
        <br>
      <?php echo 'Player ID and Team ID must be specified to update'; //playerID, teamID, contractStartDate, contractEndDate,fee
    }
    else{
      $playerID = $_POST['playerID'];
      $teamID = $_POST['teamID'];
      $number_of_fields_filled = 0;
      $fields = array("playerID","teamID","contractStartDate","contractEndDate","fee");
      ...
      if($number_of_fields_filled == 0)
      {
        ...
        <?php echo "To be updated value must be filled for at least 1 field";
          ...
          // WHERE teamID = 1;
        $sql_statement = "UPDATE `player_plays_in_team` SET ";
        ...
        foreach($fields as $field){
          if( isset($_POST[$field]) && strlen($_POST[$field]) !=0 )
          {
            ...
            if($number_of_commas > 0){
              ...
              $sql_statement .= " WHERE playerID = $playerID AND teamID = $teamID";
            }
          }
        }
        if($db->query($sql_statement) == TRUE){
      ...
      //  if none are filled then show error message  (playerID, teamID, contractStartDate, contractEndDate,fee  )
      $fields = array("playerID","teamID","contractStartDate","contractEndDate", "fee");
      $number_of_fields_filled = 0;
      ...
      $sql_statement = "SELECT * FROM `player_plays_in_team` WHERE ";
      foreach ($fields as $field){
        if (isset($_POST[$field]) && strlen($_POST[$field]) != 0  ){
