...
<body>
  ...
  else if (isset($_POST['update'])){ //(teamID, active, teamName, teamCity )
      if( ! (  (isset($_POST['teamID'])) && (strlen($_POST['teamID']) != 0))  ){
        ?>
        <body style="background-color:#f2f4f7;">
          <br>
        <?php echo 'Team ID must be specified to update';
      }
      else{
        $teamID = $_POST['teamID'];
        $number_of_fields_filled = 0;
        $fields = array("teamID","active","teamName","teamCity");
